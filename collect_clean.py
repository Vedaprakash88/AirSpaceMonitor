...
carriers = ['RFR', 'RCH', 'BAF', 'HVK', 'IAM', 'NAT', 'IAF', 'IRIAF']  # carrier list
bounds = '46.37,10.32/5' #'26.24,50.89/6' # '24.87,38.46/5' # '61.62,23.185,-18.04,54.645'  # Geo-space ==> latitude and longitudes (from Flightradar24)
today = datetime.now()
...
class SelectClean:
        ...
        if carr_frame.shape[0] == 0:
            ...
            while iter1 < carr_frame.shape[0]:
                ...
                if df2.loc["aircraft", "model"] != 'NA':
                    carr_frame.loc[carr_frame.index[iter1], ["Model"]] = df2.loc["aircraft", "model"]['text']  # ac_model
                else:
                    carr_frame.loc[carr_frame.index[iter1], ["Model"]] = "N/A"
                if df2.loc["airline", "name"] != 'NA':
                    carr_frame.loc[carr_frame.index[iter1], ["Airline"]] = df2.loc["airline", "name"]  # ac_airline
                else:
                    carr_frame.loc[carr_frame.index[iter1], ["Airline"]] = "N/A"
                if df2.loc["airport", "origin"] != 'NA':
                    carr_frame.loc[carr_frame.index[iter1], ["Airport_O"]] = df2.loc["airport", "origin"][
                        'name']  # ac_origin
                else:
                    carr_frame.loc[carr_frame.index[iter1], ["Airport_O"]] = "N/A"
                if df2.loc["airport", "destination"] != 'NA':
                    carr_frame.loc[carr_frame.index[iter1], ["Airport_D"]] = df2.loc["airport", "destination"][
                        'name']  # ac_destination
                else:
                    carr_frame.loc[carr_frame.index[iter1], ["Airport_D"]] = "N/A"

                carr_frame.loc[carr_frame.index[iter1], ["Trail"]] = df2.loc["trail"]  # ac_destination
    ...
    def build_database(carrier: str, file_name: str):
        ...
        df_car = SelectClean().carrierdata(df_fl, carrier)
...
def save_flights(df_ex: DataFrame, df_fl: DataFrame):
    ...
    df_ex: DataFrame = pd.merge(df_ex, df_fl, how='outer')
    df_ex.set_index('index', inplace=True)
